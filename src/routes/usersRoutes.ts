...
router.get("/", (req: Request, res: Response) => {
  try {
    ...
    if(!authHeader || !authHeader.startsWith("Bearer ")){
        return res.status(401).json.apply({
            success: false,
            message:"Authorization header in not found"
        })
    }
    //extract toeknt and check if toen is available
...
router.post("/login", (req: Request, res: Response) => {
  ...
  // should not push this to github cause it our key
        const jwt_secret = process.env.JWT_SECRET || "forgot_secret";
